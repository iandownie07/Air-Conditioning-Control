//
//  States.cpp
//  Implementation of methods that realise actions of the two states
...
Queue queue1; // create the queue

string output; // create a string for the queue


static volatile bool connect_flag = true;
// connect_flag allows us to signal when we should enter the "cooling" state
int eventtype_flag;
// eventtype_flag is used to signal what types of event have occurred when data is presented in laptop computer
int state_flag;
// state_flag allows us to capture button presses as log events only when we're in "idle" state
...
void on_uart_rx() {
// function with infinite loop that waits for a UART connection
...
{
 // interrupt handler that waits for a button press
    {
            ...
            
        {
            ...
            gpio_put(18,0); // set button back to 0
...
void core1_entry()
{
    ...
    while (1) {
        ...
        // advance to time after 1 second
...
// Method used to create an instance of the "Idle" state
// By declaring it "static", we only ever create a single reference to this state
{
...
void Idle::enter(AirCon* p_air)
{
    ...
    while (!gpio_get(15) || !gpio_get(10) || connect_flag == false)
        // if the door or window is open or the button hasn't been pressed, enter loop
    {
        if(!gpio_get(15) || !gpio_get(10))
        // if door or window is open, set LED
    ...
    } // we leave infinite loop if door & window are closed & button has been pressed
    p_air->toggle();
}
...
{
    // not functionally required; a placeholder
}
...
{
    state_flag = 2; // not capture button presses in "Cooling" as events
    gpio_put(17, 0);    // disconnect light
    ...
    while (gpio_get(15) && gpio_get(10)) {
        // stay cooling until a door or window opens
